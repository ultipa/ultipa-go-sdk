...
func convertSdkNodeRowToUltipaNodeRow(schema *structs.Schema, row *structs.Node, index int) (*ultipa.EntityRow, error) {
	err := checkNodeProperties(schema, row, index)
	if err != nil {
		return nil, err
	}
...
func doConvertSdkNodeRowToUltipaNodeRow(schema *structs.Schema, row *structs.Node, index int) (*ultipa.EntityRow, error) {
